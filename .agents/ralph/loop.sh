...
DEFAULT_SKILLS_MATRIX_PATH="packages/agent-orchestrator/agent-skills-matrix.json"
DEFAULT_SKILLS_GATE_SCRIPT="packages/agent-orchestrator/scripts/skills-gate.mjs"
...
SKILLS_MATRIX_PATH="${SKILLS_MATRIX_PATH:-$DEFAULT_SKILLS_MATRIX_PATH}"
SKILLS_GATE_SCRIPT="${SKILLS_GATE_SCRIPT:-$DEFAULT_SKILLS_GATE_SCRIPT}"
...
SKILLS_MATRIX_PATH="$(abs_path "$SKILLS_MATRIX_PATH")"
SKILLS_GATE_SCRIPT="$(abs_path "$SKILLS_GATE_SCRIPT")"
...
run_skill_gate() {
  ...
  if [ "$stage" = "manager" ]; then
    ...
    agent_name="$(infer_agent_name "$AGENT_CMD")"
  fi

  if [ ! -f "$SKILLS_MATRIX_PATH" ] || [ ! -f "$SKILLS_GATE_SCRIPT" ]; then
    echo "pass|(none)|(none)|(none)|skills gate artifacts not found" > "$out_file.result"
    cat "$out_file.result"
    return 0
  fi
...
status = data.get("skillGateStatus", "blocked")
missing = to_csv(data.get("skillsMissing", []))
used = to_csv(data.get("skillsUsed", []))
forbidden_used = to_csv(data.get("forbiddenSkillsUsed", []))
reasons = to_csv(data.get("reasons", []))
print(f"{status}|{missing}|{used}|{forbidden_used}|{reasons}")
PY
}
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
  ...
  SKILLS_USED="(none)"
  SKILLS_MISSING="(none)"
  SKILLS_FORBIDDEN_USED="(none)"
  SKILL_GATE_REASONS="(none)"
  ...
  if [ "$SKILL_GATE_STATUS" = "blocked" ]; then
    echo "Skill gate blocked stage '$SKILL_STAGE': $SKILL_GATE_REASONS" | tee "$LOG_FILE"
    CMD_STATUS=1
    HAS_ERROR="true"
  elif [ "${RALPH_DRY_RUN:-}" = "1" ]; then
    ...
    if [ "$CMD_STATUS" -eq 0 ]; then
      SKILL_GATE_RESULT="$(run_skill_gate "$SKILL_STAGE" "$LOG_FILE")"
      IFS='|' read -r SKILL_GATE_STATUS SKILLS_MISSING SKILLS_USED SKILLS_FORBIDDEN_USED SKILL_GATE_REASONS <<<"$SKILL_GATE_RESULT"
      if [ "$SKILL_GATE_STATUS" = "blocked" ]; then
        echo "Skill gate post-check blocked stage '$SKILL_STAGE': $SKILL_GATE_REASONS" | tee -a "$LOG_FILE"
        CMD_STATUS=1
      fi
    fi
  fi
  ...
  if [ "$MODE" = "build" ] && [ "$NO_COMMIT" = "false" ] && [ "$CMD_STATUS" -eq 0 ] && grep -q "<promise>COMPLETE</promise>" "$LOG_FILE"; then
    if [ "$HEAD_BEFORE" = "$HEAD_AFTER" ]; then
      echo "Empty commit policy violation: no new commit was created." | tee -a "$LOG_FILE"
      log_error "ITERATION $i completion blocked: no commit created"
      CMD_STATUS=1
      HAS_ERROR="true"
    elif [ -z "$CHANGED_FILES" ]; then
      echo "Empty commit policy violation: commit has no file changes." | tee -a "$LOG_FILE"
      log_error "ITERATION $i completion blocked: empty commit detected"
      CMD_STATUS=1
      HAS_ERROR="true"
    fi
  fi
