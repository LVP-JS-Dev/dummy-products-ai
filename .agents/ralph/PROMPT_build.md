...
- Required skills:
  ...
  - `react-best-practices` (for React changes)
  - `typescript-advanced-types` (for TS type changes)
- Optional skills:
  - `ui-skills`
  - `design-system-patterns`
  - `openai-docs` (only for OpenAI API work)
- Forbidden skills:
  - `yeet` (unless user explicitly requests PR flow)
  - `security-best-practices` (unless task is explicitly security-focused)
...
1. Fully audit and read all necessary files to understand the task end-to-end before implementing. Do not assume missing functionality.
1. If {{AGENTS_PATH}} exists, follow its build/test instructions.
   - If `{{AGENTS_PATH}}` is missing or stale because it is generated by Ruler, read `.ruler/agent.md` as process source-of-truth.
1. Implement only the tasks that belong to {{STORY_ID}}.
1. Run verification commands listed in the story, the global quality gates, and in {{AGENTS_PATH}} (if required).
1. If the project has a build or dev workflow, run what applies:
   - Build step (e.g., `npm run build`) if defined.
   - Dev server (e.g., `npm run dev`, `wrangler dev`) if it is the normal validation path.
   - Confirm no runtime/build errors in the console.
1. Perform a brief audit before committing:
   - **Security:** check for obvious vulnerabilities or unsafe handling introduced by your changes.
   - **Performance:** check for avoidable regressions (extra queries, heavy loops, unnecessary re-renders).
   - **Regression:** verify existing behavior that could be impacted still works.
1. If No-commit is false, commit changes using the `$commit` skill.
    - Stage everything: `git add -A`
    - Empty commits are forbidden.
    - Confirm a clean working tree after commit: `git status --porcelain` should be empty.
    - After committing, capture the commit hash and subject using:
      `git show -s --format="%h %s" HEAD`.
1. Append a progress entry to {{PROGRESS_PATH}} with run/commit/test details (format below).
...
1. Verify the UI changes work as expected.
1. Take a screenshot if helpful for the progress log.
