...
This repo additionally enforces a skills gate via:

- `packages/agent-orchestrator/agent-skills-matrix.json`
- `packages/agent-orchestrator/scripts/skills-gate.mjs`
...
## Skill gate + empty commit policy
...
Operational rules are generated from `.ruler/*`.
When changing process rules, update `.ruler` sources first, then run:
