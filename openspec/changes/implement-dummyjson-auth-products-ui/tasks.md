...
- [ ] 2.6 Add logout button/action to clear all auth-related data (token + stored username) from both storages and navigate to `/login`
- [ ] 2.7 Handle `401 Unauthorized` from protected APIs by forcing logout, redirecting to `/login`, and showing a session-expired message
...
- [ ] 3.3 Implement sortable columns (including name, price, rating) with persisted sort descriptor in screen state
- [ ] 3.4 Apply rating styling rule so values below `3` render in red
- [ ] 3.5 Implement paginated products loading via API `limit/skip` with next/prev (or page) controls and loading indicator on page change
- [ ] 3.6 Persist sort descriptor using remember-me storage policy and document “sorting applies to current page only” in the UI to avoid confusion

## 4. Search and Local Product Creation

- [ ] 4.1 Implement debounced product search input and API search request flow with retryable error handling
- [ ] 4.2 Store active search query in the URL (e.g., `q` param) and restore from URL on reload
- [ ] 4.3 Render search results in the products table while preserving active sort behavior
- [ ] 4.4 When query is empty, show default (non-search) paginated product list
- [ ] 4.5 Implement paginated search results via API `limit/skip` while query is active
...
- [ ] 5.1 Match login/products screens pixel-close to the provided Figma (excluding non-mock elements like logout)
